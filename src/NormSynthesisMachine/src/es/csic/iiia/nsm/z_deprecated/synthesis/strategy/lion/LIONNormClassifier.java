...
public class LIONNormClassifier {
	...
	public LIONNormClassifier(List<EvaluationCriteria> normEvDimensions,
...
//			this.nsMetrics.incNumNodesVisited();
	...
	private void checkEffectiveness(Norm norm,
		...
		for(Goal goal : nsmSettings.getSystemGoals()) {
			...
			{
				if(this.shouldBeDeactivated(norm,EvaluationCriteria.Effectiveness, goal)) {
			...
			{
				if(this.shouldBeActivated(norm,	EvaluationCriteria.Effectiveness, goal)) {
	...
	private void checkNecessity(Norm norm,
		...
		for(Goal goal : nsmSettings.getSystemGoals()) {
			...
			{
				if(this.shouldBeDeactivated(norm,EvaluationCriteria.Necessity, goal)) {
			...
			{
				if(this.shouldBeActivated(norm, EvaluationCriteria.Necessity, goal)) {
	...
	private void checkGeneralisation(Norm norm,
		...
		if(!isGeneralisable) {
			for(EvaluationCriteria dim : this.normEvDimensions)	 {
	...
	private void checkSubstitutability(Norm nA, Norm nB, 
		...
		for(Goal goal : nsmSettings.getSystemGoals()) {
			...
			boolean groupFFDoesNotUnderPerform =
					!this.shouldBeDeactivated(groupFF, EvaluationCriteria.Effectiveness, goal);
	 ...
	 */
	private boolean shouldBeDeactivated(NetworkNode node, EvaluationCriteria dim, Goal goal) {
		...
		float satDegree = this.nsmSettings.getNormDeactivationBoundary(dim, goal);
		float epsilon = this.nsmSettings.getNormDeactivationBoundaryEpsilon(dim, goal);
	 ...
	 * 					or necessity), and in terms of all system goals
	 */
	private boolean shouldBeActivated(NetworkNode node, EvaluationCriteria dim, Goal goal) {
		Utility utility = null;
		if(node instanceof Norm) {
			utility =  this.normativeNetwork.getUtility((Norm)node);
		}
