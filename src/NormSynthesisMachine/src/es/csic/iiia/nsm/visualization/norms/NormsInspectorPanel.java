...
public class NormsInspectorPanel extends javax.swing.JPanel {
	...
	public NormsInspectorPanel(NormSynthesisInspector nsInspector) {
		...
		miscPath = path.substring(0,path.length()-3) + "misc/normlab/icons/";
		...
		this.strategyClass = nsm.getStrategy().getClass().getSimpleName();
		
		NormSynthesisStrategy.Option strategy = this.nsmSettings.getNormSynthesisStrategy();
		String reactivity = "";
		String generalisationMode = "---";
		String generalisationStep = "---";

		int numEvidences = this.nsmSettings.getMinEvaluationsToClassifyNorms();
		reactivity = numEvidences == 1? "Purely reactive" : 
			String.valueOf(numEvidences) + " evidences";
		
		if(nsmSettings.pursueCompactness() && nsmSettings.getNormGeneralisationApproach() 
				== NormGeneraliser.Approach.Optimistic) {
			generalisationMode = nsmSettings.getOptimisticNormGeneralisationMode().toString();
			generalisationStep = String.valueOf(nsmSettings.getOptimisticNormGeneralisationStep());
		}

		if(strategy == NormSynthesisStrategy.Option.Custom) {
			this.lblNSStrategyInfo.setText(strategyClass);
		}
		else {
			this.lblNSStrategyInfo.setText(strategy.toString());
		}
		
		this.lblGenerationModeInfo.setText(reactivity);
	...
	private void updateSelectedNorm() {
		...
		double eff = utility.getScore(EvaluationCriteria.Effectiveness, goals.get(0));
		double nec = utility.getScore(EvaluationCriteria.Necessity, goals.get(0));
...
//		String loupePath = miscPath + "magnifGlass.png";
		...
		btnPerfRange.setIcon(new javax.swing.ImageIcon("misc/normlab/icons/chart.png")); // NOI18N
		...
		lblGenerationMode.setText("Reactivity:");

		lblGenerationModeInfo.setFont(new java.awt.Font("Arial", 0, 13)); // NOI18N
		lblGenerationModeInfo.setText("");
