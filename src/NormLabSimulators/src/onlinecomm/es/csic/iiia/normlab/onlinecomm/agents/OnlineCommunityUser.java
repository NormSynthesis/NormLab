 ...
 * 
 */
public class OnlineCommunityUser implements EnvironmentAgent {
	...
	private OnlineCommunityUserContext agentContext;
	private OnlineCommunityUserAction agentAction;
	private OnlineCommunityUserReasoner reasoner;
	 ...
	 */
	public OnlineCommunityUser(OnlineCommunityUserReasoner reasoner,
		...
		this.agentAction = OnlineCommunityUserAction.Nothing;
		...
		this.agentContext = new OnlineCommunityUserContext(this, factFactory);
	}
	...
	public void upLoadContent(){
		...
		if(randomNum <= uploadContentPercentage){
			...
			if(content != null) {
				...
				if(agentAction == OnlineCommunityUserAction.Upload) {
					Norm norm = reasoner.getNormToViolate();
					if(norm != null) {
						content.setViolatedNorm(norm);
					}
					doUpload(content);
					contextData.setNumSocialNetworkContents(
							contextData.getNumSocialNetworkContents() + 1);
				}	
			}
		}
	}
	...
	private boolean isContentRegulated(IContent content) {
		return this.normEngine.getNormsApplicableToContent(content).size() > 0;
	}
	...
	public boolean complaintContent(IContent content){		
		...
		if(content != null){
			...
			try{
				complaintDone = contentIsPaintedInContext(content, randomNum,
						complaintType, actualComplaintList);
			}
			catch(Exception e){
				complaintDone = contentIsNotVisibleOnContext(content, randomNum,
						complaintType, actualComplaintList);
			}
		}
	...
	public OnlineCommunityUserReasoner getReasoner(){
