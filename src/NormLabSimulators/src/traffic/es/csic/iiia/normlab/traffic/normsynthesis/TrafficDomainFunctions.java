...
public class TrafficDomainFunctions implements DomainFunctions {
	 ...
	 */
	@Override
	public EnvironmentAgentContext agentContext(long carId, View view) {
	 ...
	 */
	@Override
	public List<EnvironmentAgentAction> agentAction(long carId, 
		...
		if(goal instanceof Gcols) {
			...
			for(int x=0; x<numElems; x++) {
				...
				if(TrafficStateManager.getType(desc) == StateType.Collision ||
					...
					for(long id : ids) {
						EnvironmentAgentContext context = this.agentContext(id, tfView);
						if(context == null) {
							resolvable = false;
						}
					}
